      ...
      <div className="container mx-auto px-4 md:px-8">
        ...
        >
          ...
          <div className="flex justify-center gap-1 mb-3">
            {[...Array(5)].map((_, i) => <Star key={i} size={18} className="text-wood fill-wood" />)}
          </div>
          <p className="text-white/50 text-sm">5.0 average · Nottinghamshire homeowners &amp; businesses</p>
        </motion.div>
        ...
        <div className="max-w-4xl mx-auto mb-12">
          <AnimatePresence mode="wait">
            ...
            >
              <Quote size={36} className="text-wood/25 mb-6" />
              <blockquote className="text-white/85 text-lg md:text-xl leading-relaxed mb-8 italic font-light">
                &ldquo;{TESTIMONIALS[current].text}&rdquo;
              </blockquote>
              <div className="flex items-center gap-4 flex-wrap">
                <div className="w-12 h-12 rounded-full bg-wood/15 ring-2 ring-wood/30 text-wood font-bold flex items-center justify-center">
                ...
                <div>
                  ...
                  <div className="flex gap-0.5 mt-1">
                    {[...Array(TESTIMONIALS[current].rating)].map((_, i) => <Star key={i} size={11} className="text-wood fill-wood" />)}
                  </div>
                </div>
                <span className="ml-auto px-3 py-1 rounded-full bg-wood/15 text-wood text-xs font-medium">
            ...
            <div className="flex gap-2">
              {TESTIMONIALS.map((_, i) => (
                <button key={i} onClick={() => setCurrent(i)} aria-label={`Go to review ${i + 1}`}
                  className={`h-2 rounded-full transition-all duration-300 ${i === current ? "w-8 bg-wood" : "w-2 bg-white/20 hover:bg-white/40"}`}
                />
              ))}
            </div>
            <div className="flex gap-2">
              <button onClick={prev} aria-label="Previous"
                className="w-10 h-10 rounded-full border border-white/15 hover:border-wood hover:text-wood text-white/60 flex items-center justify-center transition-all">
                <ChevronLeft size={17} />
              </button>
          ...
          {TESTIMONIALS.slice(0, 3).map((t, i) => (
            ...
            >
              ...
              <div className="flex items-center gap-2">
                <div className="w-8 h-8 rounded-full bg-wood/15 text-wood text-xs font-bold flex items-center justify-center">
                  {initials(t.name)}
                </div>
                <div>
